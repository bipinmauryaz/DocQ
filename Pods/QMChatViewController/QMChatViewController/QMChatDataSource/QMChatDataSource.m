...
- (void)addMessage:(QBChatMessage *)message {
    //NSLog(@"chatdatasource = add message = %@",message);
    if([message.text isEqualToString:@"d0cq1ty*#@block*U"]){
        [self deleteMessage:message];
    }else if([message.text isEqualToString:@"d0cq1ty*#@unblock*U"]){
        [self deleteMessage:message];
    }else{
...
- (void)addMessages:(NSArray<QBChatMessage *> *)messages {
 //   NSLog(@"chatdatasource = add messages = %@",messages);
    dispatch_async(dispatch_get_main_queue(), ^{
        NSMutableArray *chatMessage = messages.mutableCopy;
        for(int i=0; i<chatMessage.count; i++){
            QBChatMessage *msg = [chatMessage objectAtIndex:i];
            if([msg.text isEqualToString:@"d0cq1ty*#@block*U"] || [msg.text isEqualToString:@"d0cq1ty*#@unblock*U"]){
                [self deleteMessage:msg];
                [chatMessage removeObjectAtIndex:i];
            }
        }
        [self changeDataSourceWithMessages:chatMessage forUpdateType:QMDataSourceActionTypeAdd];
    });
...
- (NSArray *)allMessages {
    for(int i=0; i<_messages.count; i++){
        QBChatMessage *msg = [_messages objectAtIndex:i];
        if([msg.text isEqualToString:@"d0cq1ty*#@block*U"] || [msg.text isEqualToString:@"d0cq1ty*#@unblock*U"]){
            [self deleteMessage:msg];
