...
//    });
    ...
    self.view_status.backgroundColor = kCOAColor;
    if([self.customUserId isEqualToString:[[NSUserDefaults standardUserDefaults]valueForKey:ownerCustId]] || [self.customUserId isEqualToString:[[NSUserDefaults standardUserDefaults]valueForKey:userId]] ){
    ...
    switch (indexPath.section) {
        case 0:
            ...
            if(isDataGet){
                ...
                ccell.lbl_total_connection.text = profileData.total_connection;
                ...
                ccell.lbl_connection_static.text = @"CONNECTIONS";
...
-(void)getProfileServiceWithCustId:(NSString*)custid{
    ...
    [[DocquityServerEngine sharedInstance]viewProfileWithAuthKey:[userPref valueForKey:userAuthKey] custom_id:custid device_type:kDeviceType ipAddress:[userPref valueForKey:ip_address] app_version:[userPref valueForKey:kAppVersion] lang:kLanguage callback:^(NSMutableDictionary *responseObject, NSError *error)
     {
...
-(void)frndRequestBtnClick{
    ...
    if ([u_permissionstus isEqualToString:@"readonly"]) {
        ...
        [self sendFriendConnectionRequest:self.profileData.user_id u_jabberId:self.userJabberId user_fname:self.profileData.first_name];
    }
     */
}
...
-(void)AcceptRequestBtnClick{
    ...
    if ([u_permissionstus isEqualToString:@"readonly"]) {
        ...
        [self AcceptFriendConnectionRequest:self.profileData.user_id u_jabberId:self.userJabberId user_fname:self.profileData.first_name];
    }
     */
}
...
-(void)ProfileViewCallWithCustomid:(NSString*)custom_id update_firstName:(NSString*)update_firstName update_lastName:(NSString *)update_lastName update_email:(NSString *)update_email update_city:(NSString*)update_city update_country:(NSString *)update_country update_state:(NSString *)update_state{
    ...
    self.navigationItem.title = [NSString stringWithFormat:@"%@ %@",update_firstName.mutableCopy,update_lastName.mutableCopy];
