...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if ([AppDelegate appDelegate].isComeFromNotificationScreen == YES) {
        [[AppDelegate appDelegate] navigateToTabBarScren:3];
    }
}
...
-(UIView *)headerView{
   ...
   // NSString*u_Img = [ImageUrl stringByAppendingPathComponent:feedPostedUserPic];
    ...
    LblUserName.text = [[NSString stringWithFormat:@"%@",[feedDict objectForKey:@"name"]?[feedDict objectForKey:@"name"]:@""]capitalizedString];
    ...
    for(int i=0; i<[assoarr count]; i++)
    {
        ...
        if ([assoarr count]==1){
            ...
            assoLbl.text = @" Everyone \uA789";
            ImgPrivacy.image = [UIImage imageNamed:@"globe.png"];
        }
    }
    ...
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(forceDownKeyboard)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.row < [[[self.commentArray objectAtIndex:indexPath.section]valueForKey:@"reply"]count])
    {
        ...
        replyCell.lbl_userName.text = [secDic valueForKey:@"name"];
        ...
        if ([resposePost isKindOfClass:[NSNull class]] || (resposePost == nil))
        {
            ...
            else if([[resposePost valueForKey:@"status"]integerValue] == 1)
            {
                ...
                if([action  isEqualToString:@"add"])
                {
                    ...
                    [commentReplyInfo setValue:[NSString stringWithFormat:@"%@ %@",[userdef objectForKey:dc_firstName],[userdef objectForKey:dc_lastName]] forKey:@"name"];
...
-(void)setCommentdata{
   ...
   // NSString *comentBy= [userpref valueForKey:userId];
    //NSString *uTyp= @"user";
...
- (void) feedCommentResponse:(NSMutableDictionary *)response
{
    ...
    if ([resposeCode isKindOfClass:[NSNull class]] || resposeCode == nil)
    {
       ...
       // NSString *total_comment=[NSString stringWithFormat:@"%@", [resposeCode objectForKey:@"total_comment"]?[resposeCode  objectForKey:@"total_comment"]:@""];
            ...
            [commentInfo setValue:[NSString stringWithFormat:@"%@ %@",[userdef objectForKey:dc_firstName],[userdef objectForKey:dc_lastName]] forKey:@"name"];
...
- (void)openProfileView:(UITapGestureRecognizer*)gesture
{
    ...
    NewProfileVC *NewProfile  = [obstoryboard instantiateViewControllerWithIdentifier:@"NewProfileVC"];
    NewProfile.userJabberId = feedjabId;
    NewProfile .customUserId=  custom_uId;
...
//    [AppDelegate appDelegate].isComeFromLeftSlider = NO;
...
//    [AppDelegate appDelegate].isOpenFromProfiletabCntlr = NO;
