...
- (void)layoutSubviews
{
    ...
    if([[_feedData valueForKey:@"speciality"]count]>0) {
        ...
        _specialityTagNameLabel.numberOfLines = 0;
    ...
    if (_showMoreButton)
    {
        ...
        if (IS_IPHONE_6_PLUS) {
            _seeMoreButton.frame = CGRectMake(x_position-padding, _lastActivityLabel.frame.size.height+14+y_poistion+_contentTextView.frame.size.height-15, [UIScreen mainScreen].bounds.size.width+300, 30.0f);
        }
        else if(IS_IPHONE_6){
            _seeMoreButton.frame = CGRectMake(x_position-padding, _lastActivityLabel.frame.size.height+14+y_poistion+_contentTextView.frame.size.height-15, [UIScreen mainScreen].bounds.size.width+250, 30.0f);
        }
        else if(IS_IPHONE_4){
            _seeMoreButton.frame = CGRectMake(x_position-padding, _lastActivityLabel.frame.size.height+14+y_poistion+_contentTextView.frame.size.height-15, [UIScreen mainScreen].bounds.size.width+180, 30.0f);
        }
        else if(IS_IPHONE_5){
            _seeMoreButton.frame = CGRectMake(x_position-padding, _lastActivityLabel.frame.size.height+14+y_poistion+_contentTextView.frame.size.height-15, [UIScreen mainScreen].bounds.size.width+180, 30.0f);
        }
    ...
    else
    {
        ...
        if([[_feedData valueForKey:@"speciality"]count]>0) {
                             _contentTextView.frame = CGRectMake(8-padding, _specialityTagNameLabel.frame.size.height+14+y_poistion+25, _containerView.frame.size.width-16, lblRect.size.height+4*padding+47);
          /*
            CGSize sizeOfText =  [_specialityTagNameLabel.text sizeWithFont:_specialityTagNameLabel.font
                                       constrainedToSize:_specialityTagNameLabel.frame.size
                                           lineBreakMode:UILineBreakModeWordWrap];
            int numberOfLines = sizeOfText.height /  _specialityTagNameLabel.font.pointSize;
            if (numberOfLines ==1) {
                _contentTextView.frame = CGRectMake(8-padding, _specialityTagNameLabel.frame.size.height+14+y_poistion+25, _containerView.frame.size.width-16, lblRect.size.height+4*padding+16);
            }
            else if (numberOfLines ==2) {
            _contentTextView.frame = CGRectMake(8-padding, _specialityTagNameLabel.frame.size.height+14+y_poistion+25, _containerView.frame.size.width-16, lblRect.size.height+4*padding+32);
             }
            else if (numberOfLines ==3) {
                 _contentTextView.frame = CGRectMake(8-padding, _specialityTagNameLabel.frame.size.height+14+y_poistion+25, _containerView.frame.size.width-16, lblRect.size.height+4*padding+47);
            }
            else if (numberOfLines ==4) {
                 _contentTextView.frame = CGRectMake(8-padding, _specialityTagNameLabel.frame.size.height+14+y_poistion+25, _containerView.frame.size.width-16, lblRect.size.height+4*padding+65);
            }
            else if (numberOfLines ==5) {
                 _contentTextView.frame = CGRectMake(8-padding, _specialityTagNameLabel.frame.size.height+14+y_poistion+25, _containerView.frame.size.width-16, lblRect.size.height+4*padding+80);
            }
           */
        }
          else{
...
//         if([[_feedData valueForKey:@"speciality"]count]>0) {
//        _feedmetaWebView.frame = CGRectMake(x_position, _lastActivityLabel.frame.size.height+14+y_poistion, _containerView.frame.size.width-2*x_position, 150.0f);
//         }
//         else{
...
- (void)setInfo:(NSDictionary*)feedInfo
{
    ...
    //Set user name
    _userNameLabel.text = [[[[NSString stringWithFormat:@"%@",[feedInfo objectForKey:@"name"]?[feedInfo objectForKey:@"name"]:@""] stringByDecodingHTMLEntities]stringByDecodingHTMLEntities]capitalizedString];
    ...
    if ([AppDelegate appDelegate].isComeFromTrending == YES) {
        ...
        if ([[feedInfo objectForKey:@"feed_kind"] isEqualToString:@"post"]) {
            _feedTypelabel.text = @"Trends";
            //_feedFilterImageView.image = [UIImage imageNamed:@"trends_hover.png"];
        }
         if ([[feedInfo objectForKey:@"feed_kind"] isEqualToString:@"cases"]) {
            _feedTypelabel.text = @"Cases";
            //_feedFilterImageView.image = [UIImage imageNamed:@"cases-hover.png"];
        }
    }
    //set User Imag
