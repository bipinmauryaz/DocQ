...
- (void)viewDidLoad {
    ...
    chatId =  [userDef objectForKey:chatId1];
    JabberName = [userDef objectForKey:kjabberName];
    jabberPassword =  [userDef objectForKey:password1];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    associationList = [[NSMutableArray alloc]init];
    self.popupParentView.hidden = YES;
    [super viewWillAppear:animated];
    [Localytics tagEvent:@"WhatsTrending"];
    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"Feed Screen Visit"];
    [self.navigationController setNavigationBarHidden:YES animated:YES];
    self.navigationItem.backBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"" style:UIBarButtonItemStylePlain target:nil action:nil];
...
-(IBAction)addFeedBtn:(id)sender{
    NSUserDefaults *userpref=[NSUserDefaults standardUserDefaults];
    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"New Feed Click"];
    ...
    if ([u_permissionstus isEqualToString:@"readonly"]) {
        ...
        if(self.myAssociationList.count==1){
            [AppDelegate appDelegate].isComeFromTrending = YES;
            postvw.myNewfeedAssociationList = self.myAssociationList.mutableCopy;
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (trends_button.selected == YES) {
        ...
        feed = [NSDictionary dictionaryWithDictionary:self.myfeedArr[indexPath.row]];
    }
    else{
        self.mycaseTV.hidden=NO;
        self.myfeedTV.hidden=YES;
    ...
    if ([seemore integerValue] == 1)
    {
        if([[feed valueForKey:@"speciality"]count]>0) {
            height += 14+55+30;//Add see more button height as well
        }
        else{
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
     if (tableView == self.myfeedTV) {
        ...
        if (cell == nil) {
            ...
            UITapGestureRecognizer *tapon_specialityLabel = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(openCommentView:)];
            [cell.specialityTagNameLabel addGestureRecognizer:tapon_specialityLabel];
            [cell.specialityTagNameLabel setUserInteractionEnabled:YES];
        ...
        if (!cell.feedImageView.hidden) {
            [cell.feedImageView setupImageViewerWithCompletionOnOpen:^{
                ...
                [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"View Feed image"];
...
{
    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"Play Video Click"];
...
{
    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"View Pdf Click"];
...
{
    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"Feed WebView Click"];
...
- (void)commentBtnClicked:(id)sender event:(id)event{
    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"Feed Comment Click"];
    [Localytics tagEvent:@"WhatsTrending Comment Click"];
    [[AppDelegate appDelegate] getPlaySound];
    NSSet *touches = [event allTouches];
    UITouch *touch = [touches anyObject];
    NSDictionary *dict;
...
- (void)shareBtnClicked:(id)sender event:(id)event{
    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"Feed Share Click"];
...
- (void)likeBtnClicked:(UIButton*)sender event:(id)event{
    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"Feed Like Click"];
    [[AppDelegate appDelegate] getPlaySound];
...
{
    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"View User Profile Click"];
        ...
        switch (buttonIndex) {
            ...
            case 1: // action edit
            {
                ...
                if (ischeckCME) {
                    ...
                    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"Feed Edit"];
                    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
                    newFeedPostViewController *postvw = [storyboard instantiateViewControllerWithIdentifier:@"NewPostFeedVC"];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag == 999){
        if (buttonIndex == 0) {
            ...
            [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"Feed Delete Click"];
...
- (void)openCommentView:(UITapGestureRecognizer*)gesture{
    [self callingGoogleAnalyticFunction:@"Feed Screen" screenAction:@"Feed Detail Click"];
    ...
    if (!isserviceCall) {
        if ([[notification name] isEqualToString:@"docquitySetFeedUploaded"]){
            ...
            if ([AppDelegate appDelegate].isCases_Feed == YES) {
                ...
                {
                    //                 [self.myfeedArr removeAllObjects];
                    [self getFeedFilteredListRequest];
                }
                isserviceCall = YES;
            }
    ...
    if(isdelete){
        if(trends_button.selected == YES){
            for(int i=0;i<myfeedArr.count ;i++){
                if([[[myfeedArr objectAtIndex:i]valueForKey:@"feed_id" ] isEqualToString:feedid])
                {
                    [myfeedArr removeObjectAtIndex:i];
                    [myfeedTV reloadData];
                    break;
                }
            }
            ...
            for(int i=0;i<myfeedArr.count ;i++){
                if([[[myfeed_casesArr objectAtIndex:i]valueForKey:@"feed_id" ] isEqualToString:feedid])
                {
                    [myfeed_casesArr removeObjectAtIndex:i];
                    [_mycaseTV reloadData];
                    break;
                }
            }
...
/*
-(void)btnKnowValidClicked:(UITapGestureRecognizer*)gesture{
    [self.tabBarController.tabBar setHidden:YES];
    [self.navigationController setNavigationBarHidden:YES animated:YES];
    [UIView animateWithDuration:1.0 animations:^{
        self.popupParentView.hidden = false;
       // [_assotableView reloadData];
    }];
}
*/

#pragma mark - open AssociationView
- (void)btnKnowValidClicked:(UITapGestureRecognizer*)gesture{
    [self.tabBarController.tabBar setHidden:YES];
    //[self.navigationController setNavigationBarHidden:YES animated:YES];
    [UIView animateWithDuration:1.0 animations:^{
    ...
    _processedFeedId =  [dict objectForKey:@"feed_id"];
    documentList = [dict objectForKey:@"association_list"];
    NSMutableString *assoImg;
   // NSMutableString *assoName;
    for(int i=0; i<[documentList count]; i++)
    {
        NSDictionary *assoInfo =  documentList[i];
         ...
         //[associationList addObject:assoName];
    }
          }];
}
