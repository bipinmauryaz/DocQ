...
- (void)viewDidLoad {
    ...
    self.lblMessage.text = [self.Msg stringByDecodingHTMLEntities];
    if([self.remarks containsString:@"pass"]){
        ...
        self.btn_transcript.hidden = YES;
        ...
        self.lblMessage.text = submitOfflineMsg;
...
- (IBAction)didPressSendResult:(id)sender {
      [self callingGoogleAnalyticFunction:@"CME Certificate Screen" screenAction:@"CME Certificate Screen Visit"];
     [self callingGoogleAnalyticFunction:@"CME Submit Screen" screenAction:@"Get Transcrpit Click"];
    ...
    webvw.accerediation = self.accerediation;
