...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if([_isShowdownloadedData isEqualToString:@"1"])
    {
        ...
        if (![self.lbl_status.text containsString:@"Complete"]) {
            ...
            [self setSubscribeCourseRequest:self.model WithType:@"preview"];
        }
    }
    }
}
...
{
    if ([AppDelegate appDelegate].ischeckSelfVerifyScreen == YES) {
        [AppDelegate appDelegate].ischeckSelfVerifyScreen = NO;
     }
    else{
...
-(void)loadData{
    NSString *selectQuery = [NSString stringWithFormat:@"SELECT _id,association_name,course_code,expiry_date,lesson_id,lesson_name,lesson_summary,lesson_description,date_of_creation,total_points,specialities_Ids,isDownload,subcription,file_type,file_url,resouce_file_name,document_name,remarks,pdf_result_url,exam_association,association,association_pic FROM cme_lesson WHERE lesson_id = %@",self.lessionID];
    if (dbResult != nil) {
        dbResult = nil;
    }
    ...
    for(NSArray *arr in dbResult){
        ...
        self.model.file_type            =   [arr objectAtIndex:indexOfFileType];
        // courseModel.association_pic     =   [arr objectAtIndex:indexOfAssopic];
        NSString *spid                  =   [arr objectAtIndex:indexOfSpIDs];
        self.model.expiry_date          =   [arr objectAtIndex:indexOfExpiryDate];
...
-(void)reloadView:(CourseModel*)courseModel{
    ...
    self.course_association.text = [self parsingHTMLText:[NSString stringWithFormat:@"%@ accredited",courseModel.association_name]];
    self.lbl_course_summary.text = [self parsingHTMLText:courseModel.lesson_summary];
    self.lbl_course_title.text = [self parsingHTMLText:courseModel.lesson_name];
    self.lbl_Course_Point.text = [self parsingHTMLText:[NSString stringWithFormat:@"Points: %@",self.coursePoint]];
    self.lbl_course_expiry.text = [self parsingHTMLText:[NSString stringWithFormat:@"Expires on : %@",[NSString setUpdateTimewithString:self.courseExpire]]];
    ...
    self.lbl_speciality.text = [self parsingHTMLText:courseModel.speciality_names];
    ...
    if ([courseModel.lesson_description isEqualToString:@""]) {
        ...
        if([self.isShowdownloadedData isEqualToString:@"1"] && [courseModel.isDownload isEqualToString: @"1"]){
            __block  NSString *htmlString = [[NSString stringWithFormat:@"<font face='Helvetica'>%@</font>", courseModel.lesson_description]stringByDecodingHTMLEntities];
            NSError *error = NULL;
            ...
            NSString *htmlString = [[NSString stringWithFormat:@"<font face='Helvetica'>%@</font>", courseModel.lesson_description]stringByDecodingHTMLEntities];
            [self.course_detail_webview loadHTMLString:htmlString baseURL: nil];
        }
    }
    if([courseModel.file_type isEqualToString:@"video"]){
        ...
        self.pfd_name.text = [self parsingHTMLText:courseModel.documentName];
     ...
     //   NSLog(@"%@",type);
        ...
        NSString *theFileName = [[courseModel.documentName lastPathComponent] stringByDeletingPathExtension];
        self.pfd_name.text = [self parsingHTMLText:theFileName];
        ...
        if([self.isShowdownloadedData isEqualToString:@"1"])
        {
            ...
            if(!self.img_pdf_thumnail.image){
                _img_pdf_thumnail.image = [UIImage imageNamed:@"pdf_placeholder.png"];
            }
    ...
    if([courseModel.isDownload isEqualToString:@"0"] || [courseModel.isDownload isEqualToString:@""] || courseModel.isDownload == nil){
        ...
        self.ProgressButton.bgViewColor = kNewCOAColor;
        UITapGestureRecognizer *solveTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(startOfflineTest)];
        ...
        if([self.lbl_status.text isEqualToString:@"Not Passed"]){
            [self callingGoogleAnalyticFunction:@"Course Detail Screen" screenAction:@"Retake Offline"];
            _ProgressButton.text = @"Retake Offline";
        }else {
            [self callingGoogleAnalyticFunction:@"Course Detail Screen" screenAction:@"Solve Offline"];
...
{
    NSString *SourceCodeString = [[webView stringByEvaluatingJavaScriptFromString:@"document.body.innerHTML"] stringByDecodingHTMLEntities];
...
- (IBAction)solveBtnAction:(id)sender {
    ...
    if ([u_permissionstus isEqualToString:@"readonly"]) {
        ...
        isOfflineSolve = NO;
        [self serviceHitForcheckUserProileDetails];
        //[self setSubscribeCourseRequest:self.model WithType:@"download"];
    }
}

//////CME Restriction
-(void)downloadStart{
 [self callingGoogleAnalyticFunction:@"Course Detail Screen" screenAction:@"Save & Solve later"];
    [Localytics tagEvent:@"CME Downloading"];
    NSUserDefaults *userpref=[NSUserDefaults standardUserDefaults];
    NSString *u_permissionstus = [userpref objectForKey:user_permission]; //mandatory
    if ([u_permissionstus isEqualToString:@"readonly"]) {
        [self getcheckedUserPermissionData];
        ...
        {
            [self singleButtonAlertViewWithAlertTitle:@"CME" message:cmeExpireMsg buttonTitle:OK_STRING];
        }
        else
        {
            isOfflineSolve = YES;
...
-(void)pushToExam{
    ...
    [AppDelegate appDelegate].isComeFromReviewListScreen = NO;
...
-(void)CourseModelUpdate:(CourseModel*)courseModel withValue:(NSDictionary*)dic{
    ...
    self.model.pdf_result_url      =   [dic valueForKey:@"pdf_url"];
    ...
    courseModel.association_name = courseModel.association_name?courseModel.association_name:[dic valueForKey:@"accreditation"];
    courseModel.accreditation_url = courseModel.accreditation_url?courseModel.accreditation_url:[dic valueForKey:@"accreditation_url"];
      [self reloadView:courseModel];
...
-(void)updateLessionData:(CourseModel*)model{
    ...
    NSString *resFileName = [self getFileNameFromURL:[resUrl stringByDeletingPathExtension]];
    self.model.resouceFname = resFileName;
    NSString *query= [NSString stringWithFormat:@"UPDATE cme_lesson SET file_type = '%@', file_url = '%@', lesson_description = '%@' ,resource_url = '%@' ,date_of_submission = '%@',resouce_file_name ='%@',document_name ='%@',thumbnail = '%@',association = '%@',accreditation = '%@' WHERE lesson_id = '%@'",model.file_type,model.file_url,lessonDesc,resUrl,model.date_of_submission,resFileName,model.documentName,model.thumbnail, model.association_ids,model.accreditation,model.lesson_id];
    [self.dbManager executeQuery:query];
...
-(void)setSubscribeCourseRequest:(CourseModel*)CModel WithType:(NSString*)type {
    [activityIndicator startAnimating];
    NSString *issample = [CModel.accreditation isEqualToString:@"Sample"]?@"1":@"0";
    ...
    NSUserDefaults *userpref=[NSUserDefaults standardUserDefaults];
    [[DocquityServerEngine sharedInstance]SubcribeCourseRequest:[userpref objectForKey:userAuthKey]lesson_id:CModel.lesson_id device_type:kDeviceType app_version:[userpref valueForKey:kAppVersion] lang:kLanguage type:type issample:issample callback:^(NSDictionary* responseObject, NSError* error)
    {
        [activityIndicator stopAnimating];
...
- (IBAction)didPressPdfOpen:(id)sender {
    ...
    [self callingGoogleAnalyticFunction:@"Course Detail Screen" screenAction:@"View Document Click"];
    [Localytics tagEvent:@"CME DeailScreen DocumentOpen Click"];
    NSString* documentfeedTitle = [NSString stringWithFormat:@"%@.pdf", self.model.documentName];
    // NSLog(@"%@",documentfeedTitle);
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    WebVC *webvw  = [storyboard instantiateViewControllerWithIdentifier:@"webVC"];
    NSString *filename = [self getFileNameFromURL:self.model.file_url];
    NSURL *filepathurl = [NSURL fileURLWithPath:[[self cmeLessonLocalPathForFile:self.model.resouceFname] stringByAppendingPathComponent:filename]];
    if([AppDelegate appDelegate].isInternet){
        webvw.fullURL = self.model.file_url;
    }else{
        webvw.fullURL = [NSString stringWithFormat:@"%@",filepathurl];
    }
...
-(void)imgTapped{
    ...
    [self callingGoogleAnalyticFunction:@"Course Detail Screen" screenAction:@"View Image Click"];
        [photoView fadeInPhotoViewFromImageView:self.img_Course];
    }
}

- (void)openVideo{
    [self callingGoogleAnalyticFunction:@"Course Detail Screen" screenAction:@"View Video Click"];
...
-(void)loadQuestionData{
    ...
    if(dbQueResult == nil || dbQueResult.count == 0){
        [self singleButtonAlertViewWithAlertTitle:@"Oops!" message:@"This course have no any questions." buttonTitle:OK_STRING];
        return;
    }
...
- (IBAction)didPressTranscript:(id)sender {
    [AppDelegate appDelegate].ischeckSelfVerifyScreen = YES;
    [self callingGoogleAnalyticFunction:@"Course Detail Screen" screenAction:@"Get Transcrpit Click"];
    ...
    if(self.model.pdf_result_url == nil || [self.model.pdf_result_url isEqualToString:@""] || [self.model.pdf_result_url isEqualToString:@"NA"] ){
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"CME" message:unableCertMsg preferredStyle:UIAlertControllerStyleAlert];
             [alertController addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
        
        [alertController addAction:[UIAlertAction actionWithTitle:@"Write Us" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [self sendWriteusMailForCourse:self.model.lesson_name];
        }]];
        [self presentViewController:alertController animated:YES completion:nil];
        // [self singleButtonAlertViewWithAlertTitle:@"CME" message:unableCertMsg buttonTitle:OK_STRING];
        return;
    }
    ...
    webvw.documentTitle = documentfeedTitle;
    webvw.accerediation = self.model.accreditation;
    //    [self updateLessionData:self.model];
    webvw.isgetResult = YES;
    [self presentViewController:webvw animated:YES completion:nil];
}

-(void)sendWriteusMailForCourse:(NSString*)courseName{
    MFMailComposeViewController *mailController = [[MFMailComposeViewController alloc] init];
    mailController.mailComposeDelegate = self;
    [mailController setSubject:@"Feedback"];
    // Fill out the email body tex
    
    NSString *emailSubject =[NSString stringWithFormat:@"Transcript required:%@",courseName];
    // Email Content
    NSString *messageBody = [NSString stringWithFormat:@"Hi,\nDocquity is unable to generate the certificate for my course %@.\nPlease send me the same.\nThanks!",courseName];
    // To address
    NSArray *toRecipents = [NSArray arrayWithObject:@"support@docquity.com"];
...
#pragma mark - Service Hit for check_user_profile_empty_fields
-(void)serviceHitForcheckUserProileDetails{
    [WebServiceCall showHUD];
    [WebServiceCall callServiceGETWithName:[NSString stringWithFormat:@"%@profile/get?rquest=check_user_profile_empty_fields&version=%@&lang=%@",NewWebUrl,kApiVersion,kLanguage] withHeader:AuthorizationAppKey postData:nil callBackBlock:^(id response, NSError *error)
     {
        if (response)
         {
             ...
             NSDictionary *postDic =[responseDict objectForKey:@"posts"];
             if ([postDic isKindOfClass:[NSNull class]] || postDic==nil)
             {
                 //tel is null
             }
               ...
               //  NSString * stusmsg =[NSString stringWithFormat:@"%@",[postDic objectForKey:@"msg"]?[postDic objectForKey:@"msg"]:@""];
                if ([[responseDict valueForKeyPath:@"posts.status"]integerValue]==1)
                 ...
                 {
                [self pushVerifyScreen:dataDic];
                }
             }
         else if ([[responseDict valueForKeyPath:@"posts.status"]integerValue]== 2)
            {
                if (isOfflineSolve==YES)
                {
                    if([AppDelegate appDelegate].isInternet)
                    {
                        _ProgressButton.text = @"Downloading...";
...
-(void)pushVerifyScreen:(NSDictionary*)verifyDict{
    [AppDelegate appDelegate].ischeckSelfVerifyScreen = YES;
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Onboarding" bundle:nil];
    VerifyYourSelfVC *selfVerify = [storyboard instantiateViewControllerWithIdentifier:@"verifyYourSelfVC"];
     selfVerify.verifyCheckDict = verifyDict;
    UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:selfVerify];
