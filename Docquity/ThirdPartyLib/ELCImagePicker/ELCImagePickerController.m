...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        ...
        if (self.maximumImagesCount==1) {
           title = [NSString stringWithFormat:NSLocalizedString(@"Only %d photo", nil), self.maximumImagesCount];
        }
        else{
             title = [NSString stringWithFormat:NSLocalizedString(@"Only %d photos", nil), self.maximumImagesCount];
        }
     
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"Maximum 4 images can be attached to a post.", nil), self.maximumImagesCount];
