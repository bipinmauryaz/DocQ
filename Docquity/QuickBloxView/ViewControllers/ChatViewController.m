...
- (void)viewDidLoad {
    ...
    chatImageView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
    chatImageView.image = [UIImage imageNamed:@"chat background"];
    self.collectionView.backgroundColor = [UIColor clearColor];
    self.collectionView.backgroundView = chatImageView;
    ...
    if (self.dialog.type == QBChatDialogTypePrivate) {
        ...
        [self.dialog setOnUserIsTyping:^(NSUInteger userID) {
         ...
         //   strongSelf.title = NSLocalizedString(@"SA_STR_TYPING", nil);
            [strongSelf navigationSetup:strongSelf.dialog.name subtitle: NSLocalizedString(@"SA_STR_TYPING", nil)];
        }];
...
- (void)refreshMessagesShowingProgress:(BOOL)showingProgress {
    ...
    [[ServicesManager instance].chatService messagesWithChatDialogID:self.dialog.ID completion:^(QBResponse *response, NSArray *messages) {
        if (response.success) {
            ...
            if ([messages count] > 0) {
               // NSLog(@"Message in chatview= %@",messages);
...
- (void)deferredQueueManager:(QMDeferredQueueManager *)queueManager didAddMessageLocally:(QBChatMessage *)addedMessage {
    ...
    if ([self.dialog.ID isEqualToString:addedMessage.dialogID]) {
      //  NSLog(@"addedMessage %@",addedMessage.text);
...
- (void)viewWillAppear:(BOOL)animated {
    [Localytics tagEvent:@"Chat Window Visit"];
...
//    UIBarButtonItem * rightitem =[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(didPressChatInfo:)];
//    self.navigationItem.rightBarButtonItem = rightitem;


    
    UIImage *moreImage = [UIImage imageNamed:@"chat_more.png"];
    UIButton *moreBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    moreBtn.backgroundColor = [UIColor clearColor];
    [moreBtn setImage:moreImage forState:UIControlStateNormal];
    moreBtn.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-40, 3, 28, 28);
    [moreBtn addTarget:self action:@selector(didPressChatInfo:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)backView
{
    ...
    if(stack.count == 1 && [[stack objectAtIndex:0]isKindOfClass:[ChatViewController class]]){
//        DialogsViewController *dialogvc;
//        NSArray * newViewControllers = [NSArray arrayWithObjects:dialogvc, [stack objectAtIndex:0],nil];
...
- (void)updateTitle {
    if (self.dialog.type == QBChatDialogTypePrivate) {
        ...
        //		if (!opponentUser) {
        ////			self.title = [opponentID stringValue];  //Nil
        //            self.title = self.dialog.name;
        //			return;
        //		}
        //        self.opponentUser = opponentUser;                 // Nil
        //        self.title = self.opponentUser.fullName;
             //self.title = self.dialog.name;
        [self navigationSetup:self.dialog.name subtitle:nil];
        //         NSUserDefaults *userdef = [NSUserDefaults standardUserDefaults];
        //         [userdef setObject:self.dialog.name forKey:@"oppositeuser"];
        //         [userdef synchronize];
    }
    else {
...
- (void)didPressSendButton:(UIButton *)button
    ...
    if (attachment.image) {
        ...
        //Should call table entry with attachment
    }
}
...
- (void)didPressSendButton:(UIButton *)button
    ...
    [self finishSendingMessageAnimated:YES];
    //Should call table entry without attachment
    [self setDialogToServer:message];
}
...
- (void)collectionView:(QMChatCollectionView *)collectionView configureCell:(UICollectionViewCell *)cell forIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cell isKindOfClass:[QMChatOutgoingCell class]]) {
        ...
        [chatCell containerView].bgColor = [UIColor colorWithRed:244.0f / 255.0f green:244.0f / 255.0f blue:244.0f / 255.0f alpha:1.0f];
    }
...
- (void)chatService:(QMChatService *)chatService didAddMessageToMemoryStorage:(QBChatMessage *)message forDialogID:(NSString *)dialogID {
  //  NSLog(@"didAddMessageToMemoryStorage");
    if ([self.dialog.ID isEqualToString:dialogID]) {
...
- (void)chatService:(QMChatService *)chatService didUpdateChatDialogInMemoryStorage:(QBChatDialog *)chatDialog {
  //  NSLog(@"didUpdateChatDialogInMemoryStorage");
...
- (void)chatService:(QMChatService *)chatService didUpdateMessage:(QBChatMessage *)message forDialogID:(NSString *)dialogID {
  //  NSLog(@"didUpdateMessage = %@",message);
...
{
    QMHeaderCollectionReusableView *footerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader
                                                                                    withReuseIdentifier:[QMHeaderCollectionReusableView cellReuseIdentifier] forIndexPath:indexPath];
    footerView.containerView.backgroundColor = [[UIColor colorWithRed:188.0/255.0 green:185.0/255.0 blue:168.0/255.0 alpha:1.0] colorWithAlphaComponent:0.5];
    blockChatBtn =  (UIButton *)[footerView viewWithTag:99];
    acceptChatBtn  = (UIButton *)[footerView viewWithTag:100];
    [blockChatBtn addTarget:self action:@selector(performChatAction:) forControlEvents:UIControlEventTouchUpInside];
    [acceptChatBtn addTarget:self action:@selector(performChatAction:) forControlEvents:UIControlEventTouchUpInside];
    return footerView;
}


-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section{
    //    NSLog(@"status = %@",self.friendStatus);
    if([self.friendStatus isEqualToString:@"8"]){
        return CGSizeMake(self.collectionView.frame.size.width, 100.0f);
    }else{
        return CGSizeZero;
    }
}
...
-(BOOL)checkDialogInTable{
   // NSLog(@"checkDialogInTable");
    recepID = 0;
    for(NSNumber *occid in self.dialog.occupantIDs){
        if(occid.integerValue != self.senderID){
            recepID = occid.integerValue;
            break;
        }
    }
    
    NSString *fetchQuery= [NSString stringWithFormat:@"SELECT dialogID,recipient_id FROM qbChatDialog WHERE recipient_id='%ld'",(long)recepID];
  ...
  //  NSLog(@"dialogInfo = %@",self.dialogInfo);
    ...
    {
        if(self.dialog.ID == [[self.dialogInfo objectAtIndex:0]objectAtIndex:0]){
            return YES;
        }else if (recepID == [[[self.dialogInfo objectAtIndex:0]objectAtIndex:indexOfDialogRecpID]integerValue]){
            return YES;
        }
    }
    return NO;
...
-(void)setDialogToServer:(QBChatMessage*)message{
    NSLog(@"setDialogToServer");
    if(![self.friendStatus isEqualToString:@"1"]){
        [self actionForUser:@"1"];
    }
    if([self checkDialogInTable]){
        [self resetUnreadCount:message.text];
        return;
    }
    NSUserDefaults *userdef = [NSUserDefaults standardUserDefaults];
   // NSLog(@"message %@",message);
   // NSLog(@"dialog =  %@",self.dialog);
    NSMutableDictionary *dic = [[NSMutableDictionary alloc]init];
    [dic setObject:self.dialog.ID forKey:@"_id"];
    //  [dic setObject:self.dialog.createdAt?self.dialog.createdAt:@"" forKey:@"created_at"];
    [dic setObject:message.text?message.text:@"" forKey:@"last_message"];
    //    [dic setObject:message.dateSent?message.dateSent:@"" forKey:@"last_message_date_sent"];
    [dic setObject:[NSString stringWithFormat:@"%lu",(unsigned long)self.senderID] forKey:@"last_message_user_id"];
    [dic setObject:self.dialog.name?self.dialog.name:@"" forKey:@"name"];
    [dic setObject:self.dialog.occupantIDs?self.dialog.occupantIDs:@"" forKey:@"occupants_ids"];
    [dic setObject:self.dialog.photo?self.dialog.photo:@"" forKey:@"photo"];
    [dic setObject:[NSString stringWithFormat:@"%lu",(unsigned long)self.dialog.type] forKey:@"type"];
    
    [dic setObject:[NSString stringWithFormat:@"%lu",(unsigned long)self.senderID] forKey:@"user_id"];
    [dic setObject:self.dialog.roomJID?self.dialog.roomJID:@"" forKey:@"xmpp_room_jid"];
    [dic setObject:@"1" forKey:@"unread_messages_count"];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss'Z'"];
    [dic setObject:self.dialog.updatedAt?[dateFormatter stringFromDate:self.dialog.updatedAt]:@"" forKey:@"updated_at"];
    NSString * timestamp = [NSString stringWithFormat:@"%lld",(long long)[message.dateSent timeIntervalSince1970]];
    [dic setObject:message.dateSent?timestamp : @"" forKey:@"last_message_date_sent"];
    [dic setObject:self.dialog.createdAt?[dateFormatter stringFromDate:self.dialog.createdAt]:timestamp forKey:@"created_at"];
    NSError *error;
    ...
    [[DocquityServerEngine sharedInstance] setDialogWithAuthkey:[userdef valueForKey:userAuthKey] oppCustID:[NSString stringWithFormat:@"%@",self.oppCustid] lang:kLanguage app_version:[userdef valueForKey:kAppVersion] dialogue:[NSString stringWithFormat:@"%@",jsonString] callback:^(NSMutableDictionary *responseObject, NSError *error) {
      //  NSLog(@"responseObject from Dialog api- %@",responseObject);
      ...
      //  NSLog(@"Response %@",responseDic);
        ...
        else
        {
            if([[responseDic valueForKey:@"status"]integerValue] == 1){
                NSMutableArray *listArr = [responseDic valueForKey:@"data"][@"list"];
                [self insertDialogs:listArr];
            }
        }
    }];
}
...
-(void)insertDialogs:(NSMutableArray*)dialog
{
    for(NSMutableDictionary *dic in dialog)
    {
        ...
        NSString *dialogPhoto         =   [dic valueForKey:@"photo"];
...
-(void)resetUnreadCount:(NSString *)lstmsg{
    NSString *dialogupdateQuery;
    NSString * timestamp = [NSString stringWithFormat:@"%lld",(long long)[[NSDate date] timeIntervalSince1970]];
    if([lstmsg isEqualToString:@""]){
        dialogupdateQuery= [NSString stringWithFormat:@"UPDATE qbChatDialog SET unread_messages_count = '0' WHERE dialogID = '%@'",self.dialog.ID];
    }else{
        dialogupdateQuery= [NSString stringWithFormat:@"UPDATE qbChatDialog SET unread_messages_count = '0',last_message_date_sent = '%@', last_message = '%@' WHERE dialogID = '%@'",timestamp?timestamp:@"",lstmsg?lstmsg:@"",self.dialog.ID];
    }
    [self.dbManager executeQuery:dialogupdateQuery];
 }

- (void)didPressChatInfo:(id)sender {
 //   NSLog(@"didPressChatInfo");
    UIAlertController * alert= [UIAlertController alertControllerWithTitle:nil message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    [alert addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
    [alert addAction:[UIAlertAction actionWithTitle:@"View Profile" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action){
    //your code here
        [self viewUserProfile];
    }]];
    //    [alert addAction:[UIAlertAction actionWithTitle:[NSString stringWithFormat:@"Block %@",_dialog.name] style:UIAlertActionStyleDestructive handler:^(UIAlertAction * action){
    //        //your code here
    //    }]];
...
-(NSString*)getCustomID{
    NSString *fetchQuery= [NSString stringWithFormat:@"SELECT full_name,custom_id FROM qbUser t1 INNER JOIN qbChatDialog t2 ON t1.qid = t2.recipient_id WHERE t2.dialogID = '%@'",self.dialog.ID];
    // Get the results.
    if (self.dialogInfo != nil) {
        self.dialogInfo = nil;
    }
    self.dialogInfo = [[NSArray alloc] initWithArray:[self.dbManager loadDataFromDB:fetchQuery]];
    NSInteger indexOfCustomID   =   [self.dbManager.arrColumnNames indexOfObject:@"custom_id"];
    // NSLog(@"dialogInfoforCustom = %@",self.dialogInfo);
    return [[self.dialogInfo objectAtIndex:0] objectAtIndex:indexOfCustomID];
}

-(void)performChatAction:(UIButton *)sender{
    if(sender.tag == 99){
        [self actionForUser:@"0"];
    }else{
        [self actionForUser:@"1"];
    }
}

-(void)actionForUser:(NSString *)action{
    NSUserDefaults *userdef = [NSUserDefaults standardUserDefaults];
    if([self.oppCustid isEqualToString:@""] || self.oppCustid == nil){
        self.oppCustid = [self getCustomID] ;
    }
    [[DocquityServerEngine sharedInstance]setUserActionWithAuthkey:[userdef valueForKey:userAuthKey] oppCustID:self.oppCustid lang:kLanguage app_version:[userdef valueForKey:kAppVersion] action:action callback:^(NSMutableDictionary *responseObject, NSError *error) {
       // NSLog(@"responseObject from Dialog api- %@",responseObject);
        NSMutableDictionary *responseDic =[responseObject objectForKey:@"posts"];
      //  NSLog(@"Response %@",responseDic);
        if ([responseDic isKindOfClass:[NSNull class]]|| responseDic ==nil)
        ...
        {
            if([[responseDic valueForKey:@"status"]integerValue] == 1){
               // NSLog(@"message for actionForUser = %@",[responseDic valueForKey:@"msg"]);
                [self updateConnectionRequest:action forCustID:self.oppCustid];
                self.friendStatus = @"1";
...
-(void)updateConnectionRequest:(NSString *)partConStatus forCustID:(NSString *)partCustID{
    NSString *userupdateQuery= [NSString stringWithFormat:@"UPDATE qbUser SET friend_status = '%@' WHERE custom_id = '%@'",partConStatus?partConStatus:@"",partCustID];
    [self.dbManager executeQuery:userupdateQuery];
}

-(void)navigationSetup:(NSString*)title subtitle:(NSString *)subtitle{
     UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
    titleLabel.backgroundColor = [UIColor clearColor];
    titleLabel.textColor = [UIColor whiteColor];
    titleLabel.font = [UIFont boldSystemFontOfSize:18];
    titleLabel.text = [[title stringByDecodingHTMLEntities]stringByDecodingHTMLEntities];
    ...
    UIView *twoLineTitleView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, MAX(subTitleLabel.frame.size.width, titleLabel.frame.size.width), 44)];
...
-(void)viewUserProfile{
    ...
    if([self.oppCustid isEqualToString:@""] || self.oppCustid == nil){
        NewProfile .custid = [self getCustomID];
    }else{
        NewProfile.custid = self.self.oppCustid;
    }
//  NewProfile.custid = [self getCustomID];
    NewProfile.comefrom = @"chat";
    self.navigationController.navigationBar.tintColor = [UIColor whiteColor];
