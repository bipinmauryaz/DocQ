...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions ID:(NSArray *)bOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple
{
    ...
    if (self = [super initWithFrame:rect])
    {
        ...
        _kDropDownOptionb = [bOptions copy];
        ...
        _kTableView.backgroundColor = [UIColor colorWithRed:0.0/255.0 green:137.0/255.0 blue:202.0/255.0 alpha:1.0];
//[UIColor clearColor];
        ...
        if (isMultipleSelection) { 
            ...
            if (IS_IPHONE_6) {
                 [btnDone setFrame:CGRectMake(rect.origin.x+225,rect.origin.y-45, 82, 25)];
            }
            else if (IS_IPHONE_6_PLUS) {
                 [btnDone setFrame:CGRectMake(rect.origin.x+260,rect.origin.y-45, 82, 25)];
            }
            else{
            [btnDone setFrame:CGRectMake(rect.origin.x+180,rect.origin.y-45, 82, 25)];
            }
...
-(void)Click_Done{
     //if (self.delegate && [self.delegate respondsToSelector:@selector(DropDownListView:Datalist:DataIdlist:)]) {
        NSMutableArray *arryResponceData=[[NSMutableArray alloc]init];
        NSMutableArray *arryResponceIdData=[[NSMutableArray alloc]init];
     //   NSLog(@"%@",self.arryData);
        for (int k=0; k<self.arryData.count; k++) {
            ...
            [arryResponceIdData addObject:[_kDropDownOptionb objectAtIndex:path.row]];
            //NSLog(@"pathRow=%ld",path.row);
           // NSLog(@" arryResponceData%@", arryResponceData);
             // NSLog(@" arryResponceIdData%@", arryResponceIdData);
           }
    [self.delegate UpdateDatalist:arryResponceData UpdateDataIdlist:arryResponceIdData];
        // [self.delegate DropDownListView:self Datalist:arryResponceData DataIdlist:arryResponceIdData];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([self.arryData containsObject:indexPath]){
        if (IS_IPHONE_6) {
            imgarrow.frame=CGRectMake(290,8, 27, 27);
        }
        else if (IS_IPHONE_6_PLUS){
             imgarrow.frame=CGRectMake(325,8, 27, 27);
        }
        else{
            imgarrow.frame=CGRectMake(245,8, 27, 27);
        }
