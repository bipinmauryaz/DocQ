...
-(BOOL)checkColumnExists
{
    ...
    if (openDatabaseResult == SQLITE_OK) {
        const char *sqlStatement = "select presenceStatus from contacts";
...
- (void)addPresenceColumnInContactsTable
{
    ...
    if (openDatabaseResult == SQLITE_OK) {
        ...
        {
            NSString *updateSQL = [NSString stringWithFormat: @"ALTER TABLE contacts ADD COLUMN presenceStatus TEXT"];
            const char *update_stmt = [updateSQL UTF8String];
            sqlite3_prepare_v2(sqlite3Database, update_stmt, -1, &statement, NULL);
            
            if(sqlite3_step(statement)==SQLITE_DONE)
            {
               // NSLog(@"Added presenceStatus column successfully");
            }
            else
            {
                //NSLog(@"Unable to alter contacts table");
            }
            // Release the compiled statement from memory
